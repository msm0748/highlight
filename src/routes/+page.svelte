<script>
	...
	const handleMouseUp = () => {
		...
		// {block_position: 1, endOffset: 4, startOffset: 1}
	...
	$: if (submitData && submitData.block_position) {
		const blocks = textContainer.querySelectorAll('p, h1, h2, h3, h4, li');
		const block = blocks[submitData.block_position];

		console.log(block, 'block');
		const text = block.textContent;
		markedText =
			text.slice(0, submitData.startOffset) +
			'<mark>' +
			text.slice(submitData.startOffset, submitData.endOffset) +
			'</mark>' +
			text.slice(submitData.endOffset);
		block.innerHTML = markedText;
	}
...
<div class="wrap">
	...
	{#if selectionRect}
		<HighlightPopup position={selectionRect} {textInfo} />
	{/if}
</div>
